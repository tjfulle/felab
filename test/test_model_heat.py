...
def test_heat_transfer_1():
    def solution(x, N=20):
        def fun(k):
            a = sin(k * pi * (1 + x[:, 0]) / 2.0) / (k ** 3 * sinh(k * pi))
            b = sinh(k * pi * (1 + x[:, 1]) / 2.0) + sinh(k * pi * (1 - x[:, 1]) / 2.0)
            return a * b

        u = (1 - x[:, 0] ** 2) / 2.0 - 16.0 / pi ** 3 * sum(
...
def test_heat_transfer_3():
    def solution(x, N=20):
        ...
        return 450.0 / pi ** 2 * sum([fun(n) for n in range(1, N)], 0)
    ...
    f2 = lambda x: where(x[:, 0] <= 2.0 / 3.0, 75 * x[:, 0], 150 * (1 - x[:, 0]))
    V = fe_model()
