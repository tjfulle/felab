...
def pyfem_elem_type(et):
    dimensions, nodes = {
        ...
        VTK_TRIANGLE: (2, 3),
        VTK_QUAD: (2, 4),
        VTK_QUADRATIC_QUAD: (2, 8),
    }[et]
    return element_family(dimensions, nodes)
...
class VTKFile(object):
    ...
    def read_mesh(self, filename):
        ...
        if not fd:
            return (coord, nodlab, elecon, elelab, elemsets, surfaces, nodesets)
        fd = fd[0]
    ...
    def write_grid(self, u=None):
        ...
        o = [
            [n for n in el[: self.eletyp[e].nodes]]
