...
def UniformBar(xa, xb, A, E, p, q, numele=10):
    ...
    numnod = numele + 1
    coord = np.linspace(xa, xb, numnod)
    elecon = np.array([[el, el + 1] for el in range(numele)])

    # LOOP OVER ELEMENTS IN THE CONNECTIVITY AND ASSEMBLE GLOBAL STIFFNESS, FORCE
    K, F = np.zeros((numnod, numnod)), np.zeros(numnod)
    for iel in range(numele):
        ...
        qe = q * he / 2.0 * np.ones(2)  # DISTRIBUTED LOAD CONTRIBUTION
    ...
    elefor, e = np.zeros(numele), np.zeros(numele)
