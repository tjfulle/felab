...
def linsolve(A, b, symmetric=True):
    ...
    if info > 0:
        logging.warn("LINSOLVE FAILED, USING LEAST SQUARES " "TO SOLVE SYSTEM")
        x = lstsq(A, F)[0]
...
def iso_dev_split2(ndir, nshr, numdim, D):
    I = array([1.0 for i in range(2)] + [0.0 for i in range(2)])
    a = dot(I, D) / numdim
    Diso = outer(a, I)
    return Diso, D - Diso
...
def axialv(a):
    ...
    """
    return 0.5 * array([a[2, 1] - a[1, 2], a[0, 2] - a[2, 0], a[1, 0] - a[0, 1]])
