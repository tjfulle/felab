...
class EXOFileWriter(EXOFile):
    ...
    def genesis(self, nodmap, elemap, coord, eleblx,
        ...
        self.fh.variables[VAR_QA_RECORDS][0, 0, :] = adjstr('pyfem2')
...
class EXOFileReader(EXOFile):
    ...
    def read(self):
        ...
        for ieb in range(numblk):
            ...
            blk = ElementBlock(name, len(eleblx)+1, elemap1[ix], elefam, blkcon)
            eleblx.append(blk)
            elemsets[name] = ix
            k += ix.shape[0]
...
def PutNodalSolution(filename, nodmap, elemap, coord, eleblx, u):
    ...
    numnod, numdim = coord.shape
    numele = len(elemap)
    exo.genesis(nodmap, elemap, coord, eleblx)
