...
def ThickPressurizedCylinderQuad4(plot=False):
    ...
    pfor = Kfac * p * a * d
    stage.assign_concentrated_load((1, 2), Rr, pfor/2.)
    stage.run()

    if plot:
        ax = V.Plot2D(deformed=1, color='orange', weight=3)
        V.Plot2D(color='blue', weight=.75, ax=ax, show=1)
    ...
    expected = array([0.054635477465204724, 0.05463547746520472,
                      0.04472949240053272, 0.044729492400532714,
                      0.04024936095840438, 0.04024936095840437,
                      0.038341661490036316, 0.03834166149003633,
                      0.03785419098608201, 0.03785419098608201])
